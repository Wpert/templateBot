...
class AsyncJsonCollection:
        ...
        if action == '$set':
            for field in _action[action]:
                if '.' in field:
                    ...
                    if isinstance(self.__json[_id][_field], list):
                        self.__json[_id][_field][int(index)] = _action[action][field]
                    elif isinstance(self.__json[_id][_field], dict):
    ...
    def __init__(self, file_path: str = "", json_object: Union[Dict[str, Any], None] = None):
    ...
    async def reload(self) -> Dict[str, Union[bool, str]]:
        """
    ...
    async def save(self) -> Dict[str, Union[bool, str]]:
        """
    ...
    async def find(self, _format: Dict[str, Any] = {}):
        """
        ...
        """
        for _id in (self.__json if isinstance (self.__json, dict) else range(len(self.__json))):
            assert isinstance(_id, str)
            verified = True
            for field in _format:
                if field in ('$first', '$last'):
                    ...
                    for real_field in _format[field]:
                        temp_verified = await query_fulfills(self.__json[_id].get(real_field, NOT_FOUND), _format[field][real_field])
                        if temp_verified:
                            break
                    verified = temp_verified
                else:
                    verified = await query_fulfills(self.__json[_id].get(field, NOT_FOUND), _format[field])
                if not verified:
                    break
            if verified:
    ...
    async def find_one(self, _format: Dict[str, Any] = {}) -> Union[Dict[str, Any], None]:
        """
    ...
    async def get(self, _id: str) -> Union[Dict[str, Any], None]:
        """
    ...
    async def delete_one(self, _format: Dict[str, Any] = {}) -> Union[Dict[str, Any], None]:
        """
    ...
    async def pop(self, _id: str) -> Union[Dict[str, Any], None]:
        """
    ...
    async def _id(self, obj: Dict[str, Any]) -> Union[str, None]:
        """
        ...
        for _id in (self.__json if isinstance(self.__json, dict) else range(len(self.__json))):
            ...
            verified = True
            for field in obj:
                verified = await query_fulfills(self.__json[_id].get(field, NOT_FOUND), obj[field])
                if not verified:
                    break
            if verified:
                return _id
    ...
    async def insert_one(self, obj: Dict[str, Any]) -> Dict[str, Union[str, bool]]:
        """
    ...
    async def update_one(self, _format: Dict[str, Any], _action: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def update_many(self, _format: Dict[str, Any], _action: Dict[str, Any]) -> Dict[str, Union[bool, str, int]]:
        """
